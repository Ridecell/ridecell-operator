...
var _ = Describe("iamrole controller", func() {
	...
	It("uses templating functionality on every possible field", func() {
		...
		iamRole.Spec.AssumeRolePolicyDocument = fmt.Sprintf(`{"Version": "2012-10-17", "Statement": [{"Effect": "Allow","Principal": {"AWS": "arn:aws:iam::%s:role/does-not-exist-{{ .Region }}"},"Action": "sts:AssumeRole"}]}`, os.Getenv("AWS_TESTING_ACCOUNT_ID"))
		...
		expectedAssumePolicy := fmt.Sprintf(`{"Version": "2012-10-17", "Statement": [{"Effect": "Allow","Principal": {"AWS": "arn:aws:iam::%s:role/does-not-exist-us-west-2"},"Action": "sts:AssumeRole"}]}`, os.Getenv("AWS_TESTING_ACCOUNT_ID"))
