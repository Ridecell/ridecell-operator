...
func (comp *appSecretComponent) Reconcile(ctx *components.ComponentContext) (components.Result, error) {
	...
	// If OTAKEYS_API_KEY is not present, add ota keys generated by mockcarservertenant kind
	apiKey, ok := appSecretsData["OTAKEYS_API_KEY"]
	if (!ok || len(apiKey.(string)) == 0) && instance.Spec.EnableMockCarServer {
